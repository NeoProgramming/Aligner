...
bool equ2(const QString& word1, const QString& word2)
{
	...
	while (i < len1 && i < len2) {
		...
		// ≈сли в любой из строк встретили '~' - дальше не сравниваем
		if (c1 == '~' || c2 == '~') {
			break;
		}

		// ≈сли символы не совпадают - строки разные
		if (c1 != c2) {
			return false;
		}
	...
	// ≈сли одна строка закончилась, а в другой на этой позиции не '~'
	if (i < len1) {
		return word1[i] == '~';
	}
	if (i < len2) {
	...
	// ќбе строки закончились одновременно
	return true;
}
...
QStringList tokenizeWords(const QString& text)
{
	...
	for (int i = 0; i < text.length(); ++i) {
		...
		// Ѕуква (любого алфавита) или цифра
...
double evaluateSentenceSimilaritySimple(const QString& sourceSentence, const QString& targetSentence)
{
	...
	// —читаем совпадени€ слов (позици€ не важна)
